...
extern "C"
int gr_geometry_primitive_cmd(lua_State* L, Geometry *g) {
  gr_node_ud* data = (gr_node_ud*)lua_newuserdata(L, sizeof(gr_node_ud));
  data->node = 0;

  const char* name = luaL_checkstring(L, 1);
  data->node = new GeometryNode( name, g );
...
int gr_sphere_cmd(lua_State* L) {
  GRLUA_DEBUG_CALL;
  return gr_geometry_primitive_cmd(L, new Sphere());
}

extern "C"
int gr_uvsphere_cmd(lua_State* L) {
  GRLUA_DEBUG_CALL;
  return gr_geometry_primitive_cmd(L, new UVSphere());
}
...
int gr_cube_cmd(lua_State* L) {
  GRLUA_DEBUG_CALL;
  return gr_geometry_primitive_cmd(L, new Cube());
}
...
int gr_light_cmd(lua_State* L)
{
  ...
  get_tuple(L, 2, &l.colour[0], 3);
  get_tuple(L, 3, &l.falloff[0], 3);
...
int gr_texture_cmd(lua_State* L) {
  ...
  auto* data = (gr_texture_ud*)lua_newuserdata(L, sizeof(gr_texture_ud));
  data->texture = 0;

  const char* name = luaL_checkstring(L, 1);
  const char* obj_fname = luaL_checkstring(L, 2);

  std::string tfname( obj_fname );

  // Use a dictionary structure to make sure every texture is loaded
  // at most once.
  auto i = texture_map.find( tfname );
  Texture *texture = nullptr;

  if( i == texture_map.end() ) {
    texture = new Texture( name, obj_fname );
  } else {
    texture = i->second;
  }
...
int gr_material_cmd(lua_State* L)
{
  ...
  data->material = new PhongMaterial(
          ...
          shininess);
  //data->material = new DebugMaterial(
  //        glm::dvec3(kd[0], kd[1], kd[2]));
