...
bool Cube::intersect(const Ray &r, float &t, glm::dvec4 &normal) const {
  glm::dvec3 e(r.from);
  glm::dvec3 d(r.v);
  ...
  for (int corner = 0; corner < 2; corner++) { // 0 or 1
    for (int planePair = 0; planePair < 3; planePair++) { // 0,1,2
      ...
      if (pp0 < 0+offst || pp0 > 1+offst) continue;
      if (pp1 < 0+offst || pp1 > 1+offst) continue;
