                            ...
                            ) const 
{
  ...
  Log::debug("outgoingRefrIdx {} incomingRefrIdx {}", outgoingRefrIdx, incomingRefrIdx);
  ...
  Log::debug("reflectedRay {} {}", to_string(reflectedRay.from), to_string(reflectedRay.v));
  glm::vec3 reflectedColor = scene.fireRay(reflectedRay);
  Log::debug("reflectedColor {}", to_string(reflectedColor));
  ...
  if (outgoingRefrIdx < incomingRefrIdx) {
    ...
    Log::debug("criticalAngle {} incomingAngle {}",
                  to_string(criticalAngle),
                  to_string(incomingAngle));
    if (incomingAngle > criticalAngle) {
      Log::debug("total internal refraction, finalColor {}", to_string(finalColor));
      return finalColor;
    }
  }
  ...
  Log::debug("refractedRay {} {}", to_string(refractedRay.from), to_string(refractedRay.v));
  
  glm::vec3 refractedColor = scene.fireRay(refractedRay);
  Log::debug("refractedColor {}", to_string(refractedColor));
